...
cut -d " " -f 2,3 AAO.txt > AAO.alleles.lower
tr '[:lower:]' '[:upper:]' < AAO.alleles.lower > AAO.alleles.upper
cut -d " " -f 1 AAO.txt > AAO.chr
cut -d " " -f 4,5,6 AAO.txt > AAO.rest
paste AAO.chr AAO.alleles.upper AAO.rest > AAO.txt
perl -p -i -e 's/\t/ /g' AAO.txt
